...
def depthFirstSearch(problem):
    ...
    "*** YOUR CODE HERE ***"
    stack = util.Stack
    root_posi = problem.getStartState()
    path = []
    visited_posi = {}

    stack.push((root_posi, path))
    while not stack.isEmpty():
        position, path = stack.pop()

        if position in visited_posi:
            continue
        if problem.isGoalState(position):
            return path

        else:
            visited_posi[position] = 1
