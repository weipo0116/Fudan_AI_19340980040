    ...
    def play(self, env, policy=None, strategy: str = "student-force"):
        ...
        for i in range(self.max_episode_len):
            if strategy == "random":
                ...
                if action_idx in ["Q", "q", "Quit", "quit"]:
                    return
                elif action_idx not in "WwDdSsAa":
                    raise IndexError
                else:
                    action = env.ACTIONS[{"W": 0, "D": 1, "S": 2, "A": 3, "w": 0, "d": 1, "s": 2, "a": 3}[action_idx]]
            else:
    ...
    def policy_evaluation(self, env, values, policy):
        while True:
            ...
            for state in env.legel_states:
                next_value = []
                prob, next_state, reward, done = env.transition(state, policy[state])[0]
                if done:
                    next_value.append(reward)
                else:

                    next_value.append(reward + self.gamma * prob * values[next_state])
                difference += abs(values[state] - max(next_value))
                values[state] = max(next_value)
            # ------ end ----------
            new_values = values
            if difference < 1e-4:
                break
        ...
        while True:
            ...
            for state in env.legel_states:
                ...
                for action in env.legal_actions(state):
                    prob, next_state, reward, done = env.transition(state, action)[0]

                    if done:
                        new_value = reward
                    else:
                        new_value = reward + self.gamma * prob * values[next_state]
                    all_value.append([new_value, action])
                all_value.sort(reverse=True)
                if all_value[0][0] > values[state]:
                    policy[state] = all_value[0][1]
                    unchanged = False
            if unchanged:
                break
            # ------ end ----------
            ...
            for state in env.legel_states:
                ...
                for action in env.legal_actions(state):
                    ...
                    if done:
                        ...
                        next_value.append(reward + self.gamma * prob * values_copy[next_state])

                values[state] = max(next_value)
                difference += abs(values_copy[state] - values[state])
            # ------ end ----------
            if difference < 1e-4:
